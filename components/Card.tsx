...
const Card: React.FC<CardProps> = ({ 
  ...
  return (
    <div className={`bg-gray-800 shadow-lg rounded-lg overflow-hidden ${className}`}>
      {title && (
        ...
        >
          <h3 className="text-lg font-semibold text-gray-100 flex justify-between items-center">
            <span>{title}</span>
            {isCollapsible && (
              <span className="text-xs text-gray-400 ml-2 p-1 rounded hover:bg-gray-600">
      ...
      {/* Parent component should control rendering of children based on isCollapsed state */}
      {/* For example, in ReportPage: {isFilterVisible && <CardContentHere />} */}
      <div className="p-4">
        {children}
      </div>
    </div>
