...
const ReportPage: React.FC = () => {
  ...
  // Use allUserProfiles from AuthContext as the source of truth
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // fetchDataFromSheet also fetches userProfiles, which are managed by useAuth
      const { transactions, trades } = await fetchDataFromSheet();
      setAllTransactions(transactions);
      setAllTrades(trades);
      setIsLoadingData(false);
    };
    // Fetch data only if auth is not loading, to ensure profiles might be ready
    if (!authIsLoading) {
        loadData();
    }
  }, [authIsLoading]); // Re-fetch if auth state changes, e.g., after login.
  ...
  const dailyPnlDataRaw = useMemo<DailyPnl[]>(() => {
    ...
    filteredTradesForPnlAndTable.forEach(trade => { 
      ...
      if (!pnlByDate[tradeDateStr]) {
        ...
        const tradeDateObj = new Date(tradeDateStr); // Date object from YYYY-MM-DD string
        let closestDateStrBefore: string | null = null;

        // Iterate through sorted keys of dailyCapitalMap to find the capital at start of day
        // This logic can be complex if dates are sparse. A simpler approach might be needed.
        // For now, if exact date match in map, use it. Otherwise, this needs more robust historical lookup.
        let capValFromMap: number | undefined = dailyCapitalMap.get(tradeDateStr);
        
        if (capValFromMap === undefined) {
          // Fallback: search for the most recent capital figure before or on this trade date
          // This requires dailyCapitalMap keys to be sortable (which they are as YYYY-MM-DD)
          const sortedCapitalDates = Array.from(dailyCapitalMap.keys()).sort();
          for (let i = sortedCapitalDates.length - 1; i >= 0; i--) {
            if (new Date(sortedCapitalDates[i]) <= tradeDateObj) {
              capValFromMap = dailyCapitalMap.get(sortedCapitalDates[i]);
              break;
            }
          }
        }
        capitalForDate = capValFromMap ?? 0; // Default to 0 if no prior capital found
  ...
  }, [filteredTradesForPnlAndTable, allTransactions, allTrades]); 
  ...
  const sortedDailyPnlData = useMemo(() => {
    ...
    if (dailyPnlSortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        if (typeof aVal === 'string' && typeof bVal === 'string') { // Dates are YYYY-MM-DD
          // Ensure correct comparison for date strings by converting to time
          if (dailyPnlSortConfig.key === 'date') {
            return dailyPnlSortConfig.direction === SortDirection.ASC 
                ? new Date(aVal).getTime() - new Date(bVal).getTime() 
                : new Date(bVal).getTime() - new Date(aVal).getTime();
          }
          // Fallback for other string types (though 'date' is the only string key in DailyPnl)
  ...
  return (
    <div className="space-y-8">
      <Card 
        ...
        onTitleClick={() => setIsFilterVisible(!isFilterVisible)}
      >
        {isFilterVisible && (
          <>
      ...
      <Card 
        title="Overall Financial Snapshot (All Time)"
        isCollapsible={true}
